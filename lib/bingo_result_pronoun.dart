...
class Bingo_pronoun_ResultScreen extends StatefulWidget {
  final int score;
  ...
  const Bingo_pronoun_ResultScreen({
    Key? key,
    required this.score,
  ...
  @override
  State<Bingo_pronoun_ResultScreen> createState() =>
      _Bingo_pronoun_ResultScreenState();
}

class _Bingo_pronoun_ResultScreenState
    extends State<Bingo_pronoun_ResultScreen> {
  ...
  Future<void> saveResultToMongoDB() async {
    ...
    if (userId != null) {
      ...
      if (userDoc == null) {
        // If user doesn't exist, insert new document with only Score_pronoun
        await userCollection.insert({
          'userId': userId,
          'week': {
            weekKey: {
              'Score_pronoun2': {
                'score_pronoun2': widget.score,
                'incorrectQuestions_pronoun2': widget.incorrectQuestions,
        ...
        // If user exists, add or update only the Score_pronoun field inside week1
        await userCollection.update(
          mongo.where.eq('userId', userId),
          mongo.modify.set('week.$weekKey.Score_pronoun2', {
            'score_pronoun2': widget.score,
            'incorrectQuestions_pronoun2': widget.incorrectQuestions,
          }),
        );
