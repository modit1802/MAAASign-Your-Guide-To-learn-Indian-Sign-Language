...
import 'package:SignEase/Challengers_All_Weeks/challenger_Week5/Review_Incorrect_challengers.dart';
...
class DetailedProgressWeek5 extends StatefulWidget {
  const DetailedProgressWeek5({super.key});

  @override
  State<DetailedProgressWeek5> createState() => _DetailedProgressWeek5State();
}

class _DetailedProgressWeek5State extends State<DetailedProgressWeek5> {
  late mongo.Db db;
  late mongo.DbCollection userCollection;
  String? score_challenger;
  String? score_adjectives;
  String? score_adverb;
  String? score_adjectives_FIB;
  String? score_bingo_adverb_2;
  ...
  Future<void> fetchResultsFromMongoDB() async {
    ...
    setState(() {
      if (result != null) {
        score_challenger = result['week']?['Week5']?['Score_Challenger_Week5']
                ?['score_challenger']
            ?.toString();
        score_adjectives = result['week']?['week5']?['Score_adjective']?['score_adjective']
            ?.toString();
        score_adverb = result['week']?['week5']?['Score_adjective2']?['score_adjective2']
            ?.toString();
        score_adjectives_FIB = result['week']?['week5']?['Score_adverb']?['score_adverb']
            ?.toString();
        score_bingo_adverb_2 = result['week']?['week5']?['Score_adverb2']?['score_adverb2']
            ?.toString();
        var data = result['week']?['Week5']?['Score_Challenger_Week5']
        ...
        score_adjectives ??= '0';
        score_adverb ??= '0';
        score_adjectives_FIB ??= '0';
        score_bingo_adverb_2 ??= '0';
        print(incorrectQuestions);
        print(score_adjectives);
        print(score_adverb);
        print(score_adjectives_FIB);
        print(score_bingo_adverb_2);
      } else {
              ...
              child: SfCartesianChart(
                ...
                series: <CartesianSeries<ScoreData, String>>[
                  ColumnSeries<ScoreData, String>(
                    dataSource: [
                      ScoreData(
                          'Adjectives', double.tryParse(score_adjectives!) ?? 0),
                    ],
                  ...
                  ColumnSeries<ScoreData, String>(
                    dataSource: [
                      ScoreData('Adverbs', double.tryParse(score_adverb!) ?? 0),
                    ],
                    xValueMapper: (ScoreData data, _) => data.label,
                    yValueMapper: (ScoreData data, _) => data.value,
                    color: Colors.blue,
              ...
              child: SfCartesianChart(
                ...
                series: <CartesianSeries<ScoreData, String>>[
                  ColumnSeries<ScoreData, String>(
                    dataSource: [
                      ScoreData(
                          'Adjectives', double.tryParse(score_adjectives_FIB!) ?? 0),
                    ],
                    xValueMapper: (ScoreData data, _) => data.label,
                    yValueMapper: (ScoreData data, _) => data.value,
                    color: Colors.red,
                  ...
                  ColumnSeries<ScoreData, String>(
                    dataSource: [
                      ScoreData('Adverbs', double.tryParse(score_bingo_adverb_2!) ?? 0),
                    ],
                    xValueMapper: (ScoreData data, _) => data.label,
                    yValueMapper: (ScoreData data, _) => data.value,
                    color: Colors.blue,
          ...
          : SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: _buildCard(
                      ...
                      description: percentage > 0.6
                          ? "Congratulations you have successfully completed Week 5."
                          : "Practice Week 5 Again",
                      index: 1,
                  ...
                  // Show the score card only if both score_adjectives and score_adverb are available
                  if (score_adjectives != null && score_adverb != null)
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: _buildScoreCard(),
                    ),
                  // Show the score card only if both score_adjectives and score_adverb are available
                  if (score_adjectives_FIB != null && score_bingo_adverb_2 != null)
                    Padding(
                      padding: const EdgeInsets.all(8.0),
